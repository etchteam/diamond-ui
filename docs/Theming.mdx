...
Diamond UI optionally includes an demo set of 3 basic themes: light, medium and dark. The theme tokens are set on the `:root` and then applied to components via classes, such as `.dmd-theme-dark`.

Components that support theming will apply the base theme tokens, such as setting the background to `--dmd-theme-background`.
